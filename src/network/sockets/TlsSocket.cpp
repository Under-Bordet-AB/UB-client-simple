...
int TlsSocket::write(const unsigned char* buf, size_t len) {
...
int TlsSocket::read(unsigned char* buf, size_t len) {
    if (!connected_) return -1;
...
bool TlsSocket::loadCACerts(const std::string& path) {
    int ret = mbedtls_x509_crt_parse_file(&cacert_, path.c_str());
    if (ret != 0) {
        Logger::error("mbedtls_x509_crt_parse_file failed: " + std::to_string(ret));
        return false;
    }
    return true;
}
